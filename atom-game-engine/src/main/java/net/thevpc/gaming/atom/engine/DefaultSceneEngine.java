...
import net.thevpc.gaming.atom.extension.SceneEngineExtension;
import net.thevpc.gaming.atom.model.armors.InvincibleSpriteArmor;
import net.thevpc.gaming.atom.model.armors.InvincibleSpriteArmorAction;
import net.thevpc.gaming.atom.model.armors.LevelSpriteArmor;
import net.thevpc.gaming.atom.model.armors.LevelSpriteArmorAction;
...
import net.thevpc.gaming.atom.model.AbstractSceneEngineModel;
...
import net.thevpc.gaming.atom.model.SceneEngineModelAdapter;
...
import net.thevpc.gaming.atom.util.MultiChronometer;
...
//    private HashMap<Integer, ModelPoint> movingSprites = new HashMap<Integer, ModelPoint>();
    ...
    protected DefaultSceneEngine() {
        ...
        spriteCollisionManagers = new SpriteExtManager<SpriteCollisionTask>(this);
        spriteTasks = new SpriteExtManager<SpriteMainTask>(this);
    }
     ...
     * {@inheritDoc}
     */
    public void addSpriteCollisionManager(SpriteCollisionTask manager) {
        if (manager == null) {
            throw new NullPointerException("SpriteCollisionTask should not be null");
        }
        globalSpriteCollisionTasks.add(manager);
    }

    /**
     * {@inheritDoc}
     */
    public void removeSpriteCollisionManager(SpriteCollisionTask manager) {
        if (manager == null) {
            throw new NullPointerException("SpriteCollisionTask should not be null");
        }
        globalSpriteCollisionTasks.remove(manager);
    }

    public void setSpriteCollisionTask(int spriteId, SpriteCollisionTask collisionManager) {
        Sprite sprite = getSprite(spriteId);
        SpriteCollisionTask old = spriteCollisionManagers.getInstanceById(spriteId);
    ...
    public void setSpriteCollisionTask(int spriteId, Class<? extends SpriteCollisionTask> collisionManager) {
        SpriteCollisionTask spriteCollisionTask = spriteCollisionManagers.create(collisionManager);
        spriteCollisionManagers.setInstanceById(spriteId, spriteCollisionTask);
    }

    public void setSpriteCollisionTask(String spriteKind, Class<? extends SpriteCollisionTask> collisionManager) {
        spriteCollisionManagers.setBeanByKind(spriteKind, collisionManager);
    }

    @Override
    public void setSpriteCollisionTask(String kind, SpriteCollisionTask collisionManager) {
        spriteCollisionManagers.setInstanceByKind(kind, collisionManager);
    }

    public SpriteCollisionTask getSpriteCollisionTask(int spriteId) {
        return spriteCollisionManagers.get(getSprite(spriteId));
    }

    @Override
    public SpriteCollisionTask getSpriteCollisionTask(String sprite) {
        return getSpriteCollisionTask((Sprite) findUniqueSpriteByName(sprite));
    }

    @Override
    public SpriteCollisionTask getSpriteCollisionTask(Sprite sprite) {
        return getSpriteCollisionTask(getSpriteId(sprite));
    }
     ...
     * by Calling <code>SpriteMainTask.nextFrame</code> on each Sprite</li>
...
//        System.out.println("###### nextFrame starting : "+getModel().getFrame());
        ...
        for (Sprite sprite : getSprites()) {
            SpriteMainTask spriteMainTask = getSpriteMainTask(sprite.getId());
            if (spriteMainTask != null) {
                if (!spriteMainTask.nextFrame(this, sprite)) {
                    setSpriteMainTask(sprite.getId(), HoldPositionSpriteMainTask.INSTANCE);
                }
            }
        }
...
//    }
    ...
    public <T extends SpriteMainTask> T getSpriteMainTask(int spriteId) {
    ...
    public void setSpriteMainTask(Sprite sprite, SpriteMainTask task) {
        setSpriteMainTask(getSpriteId(sprite), task);
    }
...
//    }
    ...
    public <T extends SpriteMainTask> T getSpriteMainTask(String name, Class<T> type) {
        return getSpriteMainTask(findUniqueSpriteByName(name).getId(), type);
    }

    @Override
    public <T extends SpriteMainTask> T getSpriteMainTask(String spriteId) {
        return getSpriteMainTask(findUniqueSpriteByName(spriteId).getId());
    }

    public <T extends SpriteMainTask> T getSpriteMainTask(Sprite sprite, Class<T> type) {
        return getSpriteMainTask(getSpriteId(sprite), type);
    }

    public <T extends SpriteMainTask> T getSpriteMainTask(Sprite sprite) {
        return getSpriteMainTask(getSpriteId(sprite));
    }

    public void setSpriteCollisionTask(Sprite sprite, SpriteCollisionTask collisionManager) {
        setSpriteCollisionTask(getSpriteId(sprite), collisionManager);
    }

    public void setSpriteMainTask(int spriteId, SpriteMainTask task) {
        Sprite sp = getSprite(spriteId);
        spriteTasks.setInstanceById(spriteId, task);

        Sprite sprite = getSprite(spriteId);
        SpriteMainTask old = spriteTasks.getInstanceById(spriteId);
        spriteTasks.setInstanceById(spriteId, task);
        if (task == null) {
            if (old != null) {
                //old.uninstall(this, sprite);
    ...
    public void setSpriteMainTask(int spriteId, Class<? extends SpriteMainTask> task) {
        Sprite sp = getSprite(spriteId);
        SpriteMainTask spriteMainTask = spriteTasks.create(task);
        setSpriteMainTask(spriteId, spriteMainTask);
    }
...
//            fireCollisionEvents(sprite, null, sprite.getLocation());
            ...
            if (kind != null) {
                SpriteMainTask instanceByKind = spriteTasks.getInstanceByKind(kind);
                if (instanceByKind != null) {
                    setSpriteMainTask(sprite.getId(), instanceByKind);
                } else {
                    Class<? extends SpriteMainTask> cls = spriteTasks.getBeanByKind(kind);
                    if (cls != null) {
                        setSpriteMainTask(sprite.getId(), spriteTasks.create(cls));
                    }
                }
                SpriteCollisionTask c_instanceByKind = spriteCollisionManagers.getInstanceByKind(kind);
                if (c_instanceByKind != null) {
                    setSpriteCollisionTask(sprite.getId(), c_instanceByKind);
                } else {
                    Class<? extends SpriteCollisionTask> c_cls = spriteCollisionManagers.getBeanByKind(kind);
                    if (c_cls != null) {
                        setSpriteCollisionTask(sprite.getId(), spriteCollisionManagers.create(c_cls));
                    }
                }
            }
        }
        ...
        public void spriteRemoving(SceneEngineModel model, Sprite sprite) {
            setSpriteMainTask(sprite.getId(), (SpriteMainTask) null);
            setSpriteCollisionTask(sprite.getId(), (SpriteCollisionTask) null);
        }
        ...
        protected void fireCollisionEvents(Sprite sprite) {
            ...
            for (Collision collision : collisionManager.detectCollisions(DefaultSceneEngine.this, sprite/*, oldLocation, newLocation*/, true, true, true)) {
                if (collision instanceof SpriteCollision) {
                    ...
                    for (SpriteCollisionTask spriteCollisionTask : globalSpriteCollisionTasks) {
                        spriteCollisionTask.collideWithBorder(sc);
                    }
                    ...
                    for (SpriteCollisionTask spriteCollisionTask : globalSpriteCollisionTasks) {
                        spriteCollisionTask.collideWithTile(sc);
                    }
                }
            }
        }
